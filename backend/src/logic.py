...
async def analyze_content(
    ...
    """
    print(
        "      - [yellow]Phase 1: Performing broad consultant-focused analysis...[/yellow]"
    )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                # --- MODIFICATION START ---
                "- 'section_title': A concise, scannable title (3-6 words max).\n"
                "- 'executive_summary': A single, hard-hitting sentence summarizing the core finding for a CEO.\n"
                # --- MODIFICATION END ---
                "- 'client_pain_points': A list of the critical business problems, challenges, or frustrations mentioned by the client.\n"
                "- 'strategic_opportunities': A list of potential business opportunities, growth levers, or new initiatives identified from the text.\n"
                "- 'key_stakeholders_mentioned': A list of key people, roles, or departments mentioned (e.g., 'CFO', 'Marketing Department').\n"
                "- 'critical_quotes': A list of 1-3 direct quotes that are perfect for putting on a slide to emphasize a point.\n"
                "- 'open_questions': A list of unresolved questions or areas needing further investigation.\n"
                "{format_instructions}",
            ),
...
async def generate_slide_deck_outline(
    ...
    """
    Generates a logical slide deck outline from the full analysis.
    """
    print("      - [blue]Generating Client-Ready Slide Deck Outline...[/blue]")

    # Consolidate the key findings from all sections into a single context
    full_context = ""
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are a Partner at a top-tier consulting firm. You are reviewing the key findings from a series of client interviews and structuring them into a compelling narrative for a final client presentation slide deck. "
                "Your task is to create a logical flow for the presentation. Start with the current situation, introduce the core challenges (pain points), present the opportunities, and conclude with high-level recommendations. "
            ...
            (
                "human",
                "Based on the following consolidated analysis, create a logical slide deck outline:\n\n--- CONSOLIDATED ANALYSIS ---\n{analysis_context}\n--- END ANALYSIS ---",
            ),
        ]
    )
    chain = prompt | llm | parser
...
async def enrich_section_with_analysis(
    ...
    try:
        print(
            f"{log_prefix} [yellow]Phase 1: Performing broad consultant-focused analysis...[/yellow]"
        )
    ...
    # --- MODIFICATION START ---
    # Use the new keys from your consultant-centric prompt
    entities_to_enrich = broad_analysis.get("key_stakeholders_mentioned", [])
    claim_to_investigate_list = broad_analysis.get("open_questions", [])
    # We only investigate one claim/question per section for the briefing
    claim_to_investigate = (
        claim_to_investigate_list[0] if claim_to_investigate_list else None
    )

    # For now, we assume key_stakeholders are our 'key_entities'.
    # The filter_entities step could be removed if the new prompt is reliable enough,
    # but for now we keep it for consistency.
    tasks_to_run = [
        filter_entities(
            entities_to_enrich, content_for_llm, runnable_config, section_index
        ),
    ]

    if job_config.get("run_contextual_briefing") and claim_to_investigate:
        # NOTE: We are no longer filtering claims, but passing the top open question directly.
        # The filter_claim function is no longer needed in this path.
        tasks_to_run.append(
            generate_contextual_briefing(
                claim_text=claim_to_investigate,
    ...
    if isinstance(task_results[0], list):
        ...
        final_analysis["key_entities"] = []  # Default to enriched stakeholders

    # Process contextual briefing results
    if job_config.get("run_contextual_briefing") and claim_to_investigate:
        briefing_result = task_results[1] if len(task_results) > 1 else None
        if isinstance(briefing_result, dict) and not briefing_result.get("error"):
            final_analysis["contextual_briefing"] = briefing_result
        else:
...
async def _process_single_section(
    ...
    """
    Helper function, refactored to fully process a single canonical section.
    It orchestrates the enrichment of the section and formats the final result for storage.
    """
    log_prefix = f"  - [Section {section_index + 1}]"
    ...
    if db_manager.does_section_result_exist(user_id, job_id, section_doc_id):
        ...
        # Still need to fetch the summary to pass up to the title logic
        existing_data = db_manager.get_section_result(user_id, job_id, section_doc_id)
        return {
            "status": "skipped",
            "executive_summary": existing_data.get("executive_summary"),
        }
    ...
    # This function now returns the fully enriched data, including the contextual briefing
    ...
    # Clip generation using the fuzzy matching function with the NEW key
    critical_quotes = analysis_data.get("critical_quotes", [])
    suggested_clips = []
    has_timestamps = start_time_s >= 0

    if has_timestamps and critical_quotes:
        print(
            f"{log_prefix} Finding timestamps for {len(critical_quotes)} critical quotes..."
        )
    ...
    # Entity enrichment for the key entities identified in the previous step
    ...
    # The contextual briefing is now included directly in analysis_data from the enrichment step
    ...
    section_result = {
        ...
        **analysis_data,  # Unpack all consultant-centric fields
    ...
    # Return the new executive_summary key for the main job title logic
    return {
        "status": "processed",
...
async def run_full_analysis(user_id: str, job_id: str):
    ...
    try:
        ...
        # Route to the correct chunking/segmentation logic
        if (
            ...
            # Use semantic boundary detection for structured transcripts
        ...
        sections = [s for s in sections if s]  # Filter out any potential empty sections
        ...
        # Find the final job title from the first valid executive summary
        final_job_title = job_doc.get("title", "Untitled Analysis")
        for result in section_processing_results:
            if (
                result
                and result.get("status") in ("processed", "skipped")
                # Change this key from "executive_summary" to "section_title"
                and result.get("section_title")
            ):
                final_job_title = result["section_title"]
        ...
        # Generate the new slide deck outline (enabled by default)
        if config.get("run_slide_deck_generation", True) and all_section_results:
            slide_deck_outline = await generate_slide_deck_outline(
                all_section_results, runnable_config
            )
            db_manager.db.collection(f"saas_users/{user_id}/jobs").document(
                job_id
            ).update({"generated_slide_outline": slide_deck_outline})
            db_manager.log_progress(
